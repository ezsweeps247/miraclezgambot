...
- **FUNDORA BLOX HOME PAGE CARD UPDATE**: Updated Fundora Blox card on home page to use new branded card design image (/game-images/fundora-blox-card.png) showing complete card layout with "FUNDORA BLOX" title, "RTP 98.5%" and "MEDIUM" volatility text. Changed bgImage in home.tsx from /game-backgrounds/fundora-blox-bg.png to /game-images/fundora-blox-card.png. Result: Professional card design with integrated branding and game information displayed directly on the card background.
- **FUNDORA BLOX CAROUSEL VERSION & HOME NAVIGATION UPDATE**: Consolidated Fundora Blox routing to use carousel version (/fundora-blox) as the main entry point. Updated carousel card to display new gameplay screenshot (/game-images/fundora-blox-new.png) showing red blocks and prize tiers. Changed mobile game Home button to navigate to home page instead of restarting game. All Fundora Blox cards throughout the app now consistently link to /fundora-blox. Files updated: game-carousel.tsx (path and image), MobileGameLayout.tsx (added useLocation hook, updated Home button navigation). Result: Unified routing, improved navigation UX, and updated carousel artwork.
- **ORIGINALS BUTTON HOVER FIX**: Fixed hover background alignment on the Originals navigation tab by removing extra leading space in the button text. The hover background now extends properly from the "O" to the "s" with perfect balance.
