...
- **FUNDORA BLOX CUSTOM CARD IMAGES**: Added custom background images for Fundora Blox game cards. Updated home page game cards to display user-provided complete card designs without overlays or additional text. Fundora Blox cards now show custom images as-is (fundora-blox-card-v2.png for home page grid, fundora-blox-carousel-v2.png for carousel). Modified home.tsx to conditionally skip overlay and text rendering for Fundora Blox cards (game.id === 'fundora-blox'). All other game cards continue to use standard overlay + text design.
- **BOTTOM NAVIGATION COLORED BORDERS**: Added unique colored top borders to 5 bottom navigation buttons for visual distinction: Menu (purple), Search (cyan), Chat (emerald), VIP/Crown (yellow), Purchase (orange). Updated CommandBar.tsx with border-t-2 classes.
