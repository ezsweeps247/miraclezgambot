    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            {/* Search Bar */}
            <div className="p-3 mt-[-7px] mb-[-7px]">
              <div className="bg-[#2a2a2a] rounded-lg flex items-center px-4 h-12 max-w-md mx-auto mt-[1px] mb-[1px] shadow-lg">
                <input
                  type="text"
                  value={searchQuery}
                  ...
                  autoFocus
                  className="flex-1 bg-transparent text-white placeholder-gray-300 outline-none text-base"
                />
                <button
                  onClick={onClose}
            ...
            <div className="mb-4 px-3">
              <Tabs value={selectedCategory} onValueChange={(value) => setSelectedCategory(value as 'all' | 'slots' | 'originals' | 'puzzle')} className="w-full">
                <TabsList className="grid w-full grid-cols-4 bg-gradient-to-r from-gray-950 via-gray-900 to-gray-950 border border-purple-500/20 rounded-lg h-12 gap-1 p-1.5">
                  <TabsTrigger 
                    value="all" 
                    data-testid="tab-all-games" 
                    className="relative text-base font-semibold text-gray-300 hover:text-white transition-all duration-200 data-[state=active]:text-white data-[state=active]:bg-purple-600 data-[state=active]:shadow-md hover:bg-[#2a2a2a] rounded-md py-2"
                  >
              ...
              <div className="overflow-x-auto hide-scrollbar mt-[23px] mb-[23px]">
                {filteredGames.length > 0 ? (
                  <div className="flex gap-3 px-3 pb-4">
                    {filteredGames.map((game) => (
                      <button
                        ...
                        className="relative group overflow-hidden rounded-xl transition-transform hover:scale-105 active:scale-95 flex-shrink-0 mt-[-12px] mb-[-12px]"
                      >
                        {/* Game Card */}
                        <div className="relative w-32 h-44 bg-gradient-to-b from-gray-800 to-gray-900 rounded-xl overflow-hidden shadow-lg">
                          {/* Game Cover Image */}
                          <div 
                          ...
                          <div className="absolute top-2 left-2 right-2">
                            <div className={`${getBadgeGradient(game.category)} backdrop-blur-md rounded-md px-2 py-1 text-center shadow-lg border border-white/30 relative overflow-hidden`}>
                              {/* Stained glass overlay effect */}
                              <div className="absolute inset-0 bg-gradient-to-br from-white/20 via-transparent to-black/10 pointer-events-none" />
                              <span className="text-xs font-bold text-white tracking-wider relative z-10 drop-shadow-md">
                                {game.provider}
                              </span>
                            </div>
                          </div>
                          ...
                          {/* Game Name */}
                          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent pt-8 pb-3 px-2">
                            <div className="text-white font-bold text-sm leading-tight text-center whitespace-pre-line">
                              {game.displayName}
                            </div>
                          </div>
                  ...
                  <div className="text-center py-16">
                    <p className="text-gray-200 text-base mb-2">No games found</p>
                    <p className="text-sm text-gray-300">Try searching with different keywords</p>
                  </div>
                )}
              </div>
            </div>
