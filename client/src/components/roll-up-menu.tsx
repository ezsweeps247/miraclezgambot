              ...
              >
                ...
                {user && wageringStatus?.hasActiveBonus ? (
                  <div className="space-y-1.5">
                    <div className="bg-black/20 rounded-lg p-2">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-white/90 text-sm font-medium">Wagering to Withdraw</span>
                      ...
                      <div className="w-full bg-black/30 rounded-full h-2 mb-1">
                        <div 
                          className="bg-gradient-to-r from-[#D4AF37] to-yellow-300 h-2 rounded-full transition-all duration-300"
                          style={{ 
                            width: `${(() => {
                              const total = wageringStatus.totalWageringRequired || 0;
                              const wagered = wageringStatus.totalWagered || 0;
                  ...
                  (user && vipStatus && vipStatus.nextLevel && vipStatus.progress < 100 && (<div className="space-y-1.5">
                    <div className="bg-black/20 rounded-lg p-2">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-white/90 font-medium text-sm">VIP Progress</span>
                      ...
                      <div className="w-full bg-black/30 rounded-full h-2 mb-1">
                        <div 
                          className="bg-gradient-to-r from-purple-400 to-purple-300 h-2 rounded-full transition-all duration-300"
              ...
              <div className="p-3">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Search"
                    className="w-full bg-[#1a1a1a] text-base text-white placeholder-gray-300 pl-10 pr-3 py-3 rounded-lg outline-none focus:ring-1 focus:ring-purple-500/50 border border-gray-700 cursor-pointer"
                ...
                {navigationItems.map((item) => (
                  <div key={item.label}>
                    <button
                      ...
                      className={`group w-full flex items-center justify-between px-3 py-3 transition-colors ${
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <item.icon className={`w-5 h-5 transition-colors ${item.isPurple ? 'text-purple-500 group-hover:text-purple-600' : 'text-gray-300 group-hover:text-purple-600'}`} />
                        <span className="transition-colors text-white group-hover:text-purple-600 text-base">{item.label}</span>
              ...
              <div className="pb-2">
                {gameCategories.map((item) => (
                  <button
                    ...
                    className="group w-full flex items-center gap-3 px-3 py-3 hover:bg-[#1a1a1a] transition-colors"
                    data-testid={`button-menu-${item.label.toLowerCase().replace(/\s+/g, '-')}`}
                  >
                    {item.isComponent ? (
                      <item.icon className="w-5 h-5 text-gray-300 transition-colors group-hover:text-purple-600" />
                    ) : (
                      <item.icon className="w-5 h-5 text-gray-300 transition-colors group-hover:text-purple-600" />
                    )}
                    <span className="text-white transition-colors group-hover:text-purple-600 text-base">{item.label}</span>
                  </button>
                ))}
              </div>
                ...
                {mainMenuItems.map((item) => (
                  <div key={item.label}>
                    <button
                      ...
                      className="group w-full flex items-center justify-between px-3 py-3 hover:bg-[#1a1a1a] transition-colors"
                      data-testid={`button-menu-${item.label.toLowerCase().replace(/\s+/g, '-')}`}
                    >
                      <div className="flex items-center gap-3">
                        <item.icon className="w-5 h-5 text-gray-300 transition-colors group-hover:text-purple-600" />
                        ...
                        >
                          {item.submenu.map((subItem) => (
                            <button
                              ...
                              className="group w-full flex items-center gap-3 px-10 py-3 hover:bg-[#1a1a1a] transition-colors"
                              data-testid={`button-menu-${subItem.label.toLowerCase()}`}
                            >
                              <subItem.icon className="w-4 h-4 text-gray-300 transition-colors group-hover:text-purple-600" />
                              <span className="text-gray-200 transition-colors group-hover:text-purple-600 text-base">{subItem.label}</span>
                            </button>
                          ))}
                        </motion.div>
                      </AnimatePresence>
