      ...
      if (isTelegram) {
        ...
        try {
          ...
          const attemptAuth = () => {
            ...
            if (initData) {
              ...
              if (import.meta.env.DEV || window.location.hostname === 'localhost' || window.location.hostname.includes('replit')) {
                ...
                // Still try test authentication if we appear to be in development
                if (!window.Telegram?.WebApp?.initData) {
                  console.log('⚠️ No Telegram data found, attempting test authentication...');
                  loginMutation.mutate('test-miraclez-2025');
                }
          ...
          if (import.meta.env.DEV || window.location.hostname === 'localhost' || window.location.hostname.includes('replit')) {
            ...
            // Production: Telegram init error - but still try test auth as fallback
            ...
            console.log('⚠️ Attempting test authentication as fallback...');
            loginMutation.mutate('test-miraclez-2025');
            setIsInitialized(true);
          }
        }
