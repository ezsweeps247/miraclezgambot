...
// Admin access group - allows all admin users
class AdminAccessGroup extends BaseObjectAccessGroup {
  constructor(id: string) {
    super(ObjectAccessGroupType.ADMIN, id);
  }

  async hasMember(userId: string): Promise<boolean> {
    // In a real implementation, check if user is an admin
    // For now, return true for admin group access
    return true;
  }
}
...
function createObjectAccessGroup(
  ...
  switch (group.type) {
    case ObjectAccessGroupType.ADMIN:
      return new AdminAccessGroup(group.id);
